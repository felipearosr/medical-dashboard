            ...
            {projections.map((proj, index) => (
              <Card key={index} className="bg-white">
                <CardContent className="p-6">
                  <p className="text-muted-foreground text-sm">{proj.period}</p>
                  <p className="text-3xl font-bold text-primary mt-2">{proj.value}</p>
                  <p className="text-sm text-muted-foreground mt-1">Confianza: {proj.confidence}</p>
                </CardContent>
              </Card>
            ))}
          </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {insights.map((insight, index) => (
            <Alert key={index} variant={insight.type === 'info' ? 'default' : insight.type as any}>
              <div className={`
